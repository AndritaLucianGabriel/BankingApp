...
## **Operations Package**
#### *1. CurrencyExchange*
 The class provides a service that allows the transfer of funds between two different accounts whatever their type of currency.
...
## **Service Packge**
#### *1. AccountStatement*
 The class will be heavily modified later and will be used later to generate an account statement based on some filters.
   1. balanceCheck(double value, String currency)
      > Method checks the account's balance.
#### *2. FormatDouble*
...
#### *1. BankAccountValidation*
 Service class that checks all the account's information to match the specific template and reduce the chance to insert faulty data.
#### *2. BankValidation*
 Service class that checks all the bank's information to match the specific template and reduce the chance to insert faulty data.
#### *3. CardValidation*
 Service class that checks all the card's information to match the specific template and reduce the chance to insert faulty data.
#### *4. ClientValidation*
 Service class that checks all the client's information to match the specific template and reduce the chance to insert faulty data. 
#### *5. LoanValidation*
 Service class that checks all the loan's information to match the specific template and reduce the chance to insert faulty data. 
#### *6. ToProvidersValidation*
 Service class that checks all the provider's information to match the specific template and reduce the chance to insert faulty data. 
## **Main classes**
#### *1. Bank*
 The class stores details like the bank's name and location and uses two maps to store the information regarding it's clients, their bank accounts and loans.
   ...
   10. removeCard(String cardNumber)
       > Method that removes a card from a bank account based on it's card number.
   ...
   12. removeClientLoan(String cnp)
       > Method that removes all the client's loans from the bank's storage system based on his cnp.
   ...
   15. removeClientBankAccount(String cnp)
       > Method that removes all the client's bank accounts from the bank's storage system based on his cnp.
   ...
   19. interBanking(String receiver, String sender, double value)
       > Method that allows a client to transfer funds from one of his bank accounts to another client. The method will convert the ammount to his receiver's bank account's type of currency based on a conversion ratio.
   ...
   21. paymentUtilies(String Sender, String Receiver, double value)
       > Method that allows a client to pay his utilities using an existing DB in the bank systems. The method will convert the ammount to his receiver's bank accounts's type of currency based on a conversion ratio.
...
#### *2. BankAccount*
 The class creates a unique ID of the account in relation to the client and then stores data about the client's money, the type of currency he prefers and a list of all the cards that were linked to this account.
   1. addCard(Card card)
   ...
   3. withdraw(double value)
      > Method that withdraws an ammount from the selected account.
   4. deposit(double value)
      > Method that deposits an ammount into the selected account.
   5. paymentUtilies(String IBAN, double value)
      > Method that pays to one of the bank's registered providers an ammount. The method checks for a difference in the accounts' currency and automatically converts the value to match the receiver.
